 ...
 */
class MessageSelector implements MessageSelectorInterface
{
    /**
     * {@inheritdoc}
     */
     ...
     * @return mixed
     */
    protected function extractFromString(string $part, int|float $number): mixed
    {
     ...
     */
    protected function stripConditions(array $segments): array
    {
        $results = [];

        foreach ($segments as $part) {
            $results[] = preg_replace('/^[\{\[]([^\[\]\{\}]*)[\}\]]/', '', $part);
        }
     ...
     * @return int
     */
    protected function getPluralIndex(string $locale, int|float $number): int
    {
        return match ($locale) {
